...
public class RayCast {
   public static MovingObjectPosition rayCastBlocks(World world, Vec3 start, Vec3 end, BiPredicate<Block, Integer> canCollide) {
      if (Double.isNaN(start.xCoord) || Double.isNaN(start.yCoord) || Double.isNaN(start.zCoord)) {
         return null;
      } else if (!Double.isNaN(end.xCoord) && !Double.isNaN(end.yCoord) && !Double.isNaN(end.zCoord)) {
         ...
         for (int step = 200; step >= 0; step--) {
            ...
            if (canCollide.test(currentBlock, currentMeta)) {
               MovingObjectPosition hit = currentBlock.collisionRayTrace(world, currentX, currentY, currentZ, start, end);
               if (hit != null) {
                  return hit;
               }
            }
