...
public class ItemInstance<S extends ManagedState<S>> extends UniversalObject {
   ...
   public ItemStack getItemStack() {
      return this.player instanceof EntityPlayer ? ((EntityPlayer)this.player).inventory.getStackInSlot(this.itemInventoryIndex) : null;
   }
   ...
   public static int getAmmo(ItemStack itemStack) {
      return itemStack != null && itemStack.stackTagCompound != null ? itemStack.stackTagCompound.getInteger("Ammo") : 0;
   }
   ...
   public static void setAmmo(ItemStack itemStack, int ammo) {
      if (itemStack != null) {
         ...
         itemStack.stackTagCompound.setInteger("Ammo", ammo);
      }
   }
      ...
      if (itemStack != null) {
         ...
         if (instance != null) {
            ...
            try {
               ...
               buf.readBytes(bytes);
               itemStack.stackTagCompound.setByteArray("Instance", bytes);
            } finally {
               buf.release();
            }
         } else {
   ...
   private static ItemInstance<?> deserializeInstance(ItemStack itemStack) {
      if (itemStack != null && itemStack.stackTagCompound != null) {
         byte[] bytes = itemStack.stackTagCompound.getByteArray("Instance");
