...
public class WeaponRenderHandler {
   ...
   public void onRenderTick(RenderTickEvent event) {
      ...
      ShaderContext shaderContext = new ShaderContext(ShaderPhase.WORLD_RENDER, minecraft.entityRenderer, minecraft.getFramebuffer(), event.renderTickTime);
   ...
   public void onRenderHand(RenderHandEvent event) {
      if (ScopePerspective.isRenderingScope) {
         ...
         if (minecraft.gameSettings.thirdPersonView == 0) {
            ItemWeaponInstance weaponInstance = ItemInstanceRegistry.getMainHeldWeapon();
            ShaderContext shaderContext = new ShaderContext(ShaderPhase.ITEM_RENDER, null, minecraft.getFramebuffer(), event.partialTicks);
   ...
   public void onTextureStitch(net.minecraftforge.client.event.TextureStitchEvent.Pre event) {
      event.map.registerIcon(new ResourceLocation("newgunrizons", "particle/blood").toString());
   }
}
