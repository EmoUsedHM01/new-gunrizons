...
public class Explosion {
   ...
   public void doExplosionA() {
      ...
      int minX = MathHelper.floor_double(this.explosionX - blastDiameter - 1.0);
      int maxX = MathHelper.floor_double(this.explosionX + blastDiameter + 1.0);
      int minY = MathHelper.floor_double(this.explosionY - blastDiameter - 1.0);
      int maxY = MathHelper.floor_double(this.explosionY + blastDiameter + 1.0);
      int minZ = MathHelper.floor_double(this.explosionZ - blastDiameter - 1.0);
      int maxZ = MathHelper.floor_double(this.explosionZ + blastDiameter + 1.0);
      List<Entity> nearbyEntities = this.world.getEntitiesWithinAABBExcludingEntity(this.exploder, AxisAlignedBB.getBoundingBox(minX, minY, minZ, maxX, maxY, maxZ));
      Vec3 explosionCenter = Vec3.createVectorHelper(this.explosionX, this.explosionY, this.explosionZ);

      for (Entity entity : nearbyEntities) {
         double relativeDistance = entity.getDistance(this.explosionX, this.explosionY, this.explosionZ) / blastDiameter;
         if (relativeDistance <= 1.0) {
            double deltaX = entity.posX - this.explosionX;
            double deltaY = entity.posY + entity.getEyeHeight() - this.explosionY;
            ...
            if (distance != 0.0) {
               ...
               entity.attackEntityFrom(
                  DamageSource.setExplosionSource(this.getCompatibleExplosion()), (int)((impact * impact + impact) / 2.0 * 7.0 * blastDiameter + 1.0)
               );
               ...
               if (entity instanceof EntityPlayer) {
                  ...
                  if (!entityplayer.capabilities.isCreativeMode || !entityplayer.capabilities.isFlying) {
                     this.playerKnockbackMap.put(entityplayer, Vec3.createVectorHelper(deltaX * impact, deltaY * impact, deltaZ * impact));
                  }
               }
            }
         }
   ...
   public void doExplosionB(boolean spawnParticles) {
      ...
      if (this.isSmoking) {
         for (BlockPos blockpos : this.affectedBlockPositions) {
            ...
            if (spawnParticles) {
               for (int i = 0; i < 3; i++) {
                  double particleX = blockpos.getX() + this.world.rand.nextFloat();
                  double particleY = blockpos.getY() + this.world.rand.nextFloat();
                  double particleZ = blockpos.getZ() + this.world.rand.nextFloat();
                  double offsetX = particleX - this.explosionX;
                  double offsetY = particleY - this.explosionY;
                  double offsetZ = particleZ - this.explosionZ;
                  double distance = MathHelper.sqrt_double(offsetX * offsetX + offsetY * offsetY + offsetZ * offsetZ);
                  offsetX /= distance;
                  offsetY /= distance;
                  offsetZ /= distance;
            ...
            if (block.getMaterial() != Material.air) {
               if (block.canDropFromExplosion(this.getCompatibleExplosion())) {
                  int meta = this.world.getBlockMetadata(blockpos.getX(), blockpos.getY(), blockpos.getZ());
                  block.dropBlockAsItemWithChance(this.world, blockpos.getX(), blockpos.getY(), blockpos.getZ(), meta, 1.0F / this.explosionSize, 0);
               }
         ...
         if (spawnParticles) {
            for (int i = 0; i < 15; i++) {
               double pX = this.explosionX + this.world.rand.nextGaussian() * 0.8;
               double pY = this.explosionY + this.world.rand.nextGaussian() * 0.9;
               double pZ = this.explosionZ + this.world.rand.nextGaussian() * 0.8;
               double motionX = this.world.rand.nextGaussian() * 0.001;
               double motionY = this.world.rand.nextGaussian() * 1.0E-4;
               double motionZ = this.world.rand.nextGaussian() * 0.001;
               ParticleManager.spawnExplosionSmoke(
                  pX, pY, pZ, motionX, motionY, motionZ, 1.0F, 250 + (int)(this.world.rand.nextFloat() * 30.0F), SMOKE_TEXTURE
               );
            }
         }
      }
      ...
      if (this.isFlaming) {
         for (BlockPos blockpos : this.affectedBlockPositions) {
            if (this.world.getBlock(blockpos.getX(), blockpos.getY(), blockpos.getZ()).getMaterial() == Material.air
               && this.world.getBlock(blockpos.getX(), blockpos.getY() - 1, blockpos.getZ()).isOpaqueCube()
