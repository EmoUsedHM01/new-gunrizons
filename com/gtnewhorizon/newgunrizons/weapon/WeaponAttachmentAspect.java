...
public final class WeaponAttachmentAspect implements Aspect<WeaponState, ItemWeaponInstance> {
   ...
   private final Predicate<ItemWeaponInstance> clickSpammingPreventer = es -> System.currentTimeMillis() >= es.getStateUpdateTimestamp() + 150L;
   private final Predicate<ItemWeaponInstance> clickSpammingPreventer2 = es -> System.currentTimeMillis() >= es.getStateUpdateTimestamp() + 150L * 2L;
   ...
   public List<CompatibleAttachment> getActiveAttachments(EntityLivingBase player, ItemStack itemStack) {
      ...
      if (!(itemInstance instanceof ItemWeaponInstance)) {
         ...
         for (CompatibleAttachment attachment : ((ItemWeapon)itemStack.func_77973_b()).getCompatibleAttachments().values()) {
      ...
      for (int activeIndex : activeAttachmentsIds) {
         if (activeIndex != 0) {
            Item item = Item.func_150899_d(activeIndex);
            if (item instanceof ItemAttachment) {
               CompatibleAttachment compatibleAttachment = weapon.getCompatibleAttachments().get(item);
   ...
   private void changeAttachmentAction(ItemWeaponInstance weaponInstance) {
      ...
      if (attachmentCategory != null) {
         NewGunrizonsMod.CHANNEL.sendToServer(new WeaponActionMessage((byte)3, weaponInstance.getItemInventoryIndex(), (byte)attachmentCategory.ordinal()));
   ...
   private void changeAttachmentInternal(AttachmentCategory attachmentCategory, ItemWeaponInstance weaponInstance) {
      if (weaponInstance.getPlayer() instanceof EntityPlayer) {
         ...
         if (currentAttachment != null) {
            CompatibleAttachment currentCompatibleAttachment = weaponInstance.getWeapon().getCompatibleAttachments().get(currentAttachment);
            if (currentCompatibleAttachment.isPermanent()) {
               return;
            }
         }

         WeaponAttachmentAspect.AttachmentLookupResult lookupResult = this.next(attachmentCategory, currentAttachment, weaponInstance);
         if (currentAttachment != null && currentAttachment.getRemoveHandler() != null) {
            currentAttachment.getRemoveHandler().apply(currentAttachment, weaponInstance);
         }
         ...
         if (lookupResult.index >= 0) {
            ...
            if (nextAttachment.getApplyHandler() != null) {
               nextAttachment.getApplyHandler().apply(nextAttachment, weaponInstance);
               ...
               ItemAttachment.AttachmentHandler handler = weaponInstance.getWeapon().getEquivalentHandler(attachmentCategory);
               if (handler != null) {
   ...
   private WeaponAttachmentAspect.AttachmentLookupResult next(AttachmentCategory category, Item currentAttachment, ItemWeaponInstance weaponInstance) {
      ...
      if (originallySelectedAttachmentIndexes != null && originallySelectedAttachmentIndexes.length == AttachmentCategory.VALUES.length) {
         ...
         for (int i = 0; i < 37; i++) {
            ...
            if (slotItemStack != null && slotItemStack.func_77973_b() instanceof ItemAttachment) {
               ItemAttachment attachmentItemFromInventory = (ItemAttachment)slotItemStack.func_77973_b();
   ...
   public static void addAttachment(ItemAttachment attachment, ItemWeaponInstance weaponInstance) {
      ...
      if (currentAttachment == null) {
         if (attachment != null && attachment.getApplyHandler() != null) {
            attachment.getApplyHandler().apply(attachment, weaponInstance);
         }
   ...
   public static ItemAttachment getActiveAttachment(AttachmentCategory category, ItemWeaponInstance weaponInstance) {
      ...
      for (int activeIndex : activeAttachmentIds) {
         if (activeIndex != 0) {
            Item item = Item.func_150899_d(activeIndex);
            if (item instanceof ItemAttachment) {
               CompatibleAttachment compatibleAttachment = weaponInstance.getWeapon().getCompatibleAttachments().get(item);
   ...
   public static boolean isActiveAttachment(ItemAttachment attachment, ItemWeaponInstance weaponInstance) {
      int[] activeAttachmentIds = weaponInstance.getActiveAttachmentIds();
      return Arrays.stream(activeAttachmentIds).anyMatch(attachmentId -> attachment == Item.func_150899_d(attachmentId));
   }
